...
Milestone 17 adds a separate organisational-memory isolation boundary. Memories retain origin and visibility scopes, and broader reuse creates a new reviewed abstraction rather than widening the source record. Forced row-level security hides restricted memories, evidence, relationships and promotion lineage; person memory remains owner-only. These controls do not define the future host product's project or client membership. Client-wide propagation must remain disabled until that system of record and its joiner/mover/leaver behavior are integrated and independently tested. See the [memory isolation contract](./docs/memory-isolation-contract.md).

Milestone 18 imports project membership through a narrow host-product binding and enforces it alongside ordinary Resource scopes. Capture runs, schedules, jobs, reviews and kickoff packs all use forced RLS. The `client_memory_enabled` field is constrained to `false`; application code cannot opt into client-wide reuse. The public adapter and fixture are synthetic. A real adapter must verify membership revision, removals and project/client identity before it can be trusted. See the [project-memory workflow](./docs/project-memory-workflow.md).

Milestone 19 anchors every context asset to an access-controlled canonical Resource and applies forced RLS to assets, provenance, dependencies, metric definitions and quality receipts. Dependency endpoints must share an exact access scope so a visible asset or quality receipt cannot imply a hidden dependency. Provenance may remain more restricted, but its association is returned only when both the asset and source Resource are visible.

Milestone 20 stores only simulated source labels and operations; it contains no vendor credential, token or live endpoint. Source artifacts inherit an access scope before perception, and derived runs and observations must match that exact boundary. Discovery candidates are rejected unless every evidence Resource shares the candidate's exact scope, and RLS requires the actor to see every evidence Resource before returning the candidate. Forced RLS also requires active project membership. Entitlement revisions and deletion modes are durable contracts, but a real connector must prove provider-specific revocation, group expansion, tombstones and backfills before handling organisational data. Never replace the simulated adapter with a credential embedded in source code or client-side configuration.
