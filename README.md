...
Open **Continual hypothesis + memory loop** in the persisted-state panel. The drawer begins with a unified hypothesis index: configured monitors and newly discovered candidates share one lifecycle, evidence and review vocabulary while retaining their origin. It then shows the monitored hypothesis, permission-scoped execution contract, source trigger, before/after context snapshots, evidence delta and newly formed counter-hypothesis. Alex can accept it as durable memory or dismiss it while retaining the audit trail. These demo review writes are local-only and disabled in production.

The **Unprompted discovery** lane shows a supplier-onboarding example: unlabeled source records flow through an ontology-guided pattern scan to an explicitly untrusted hypothesis candidate and human review gate. Alex can run a normal background scan, pause or resume its six-hour formation schedule, accept and start candidate-specific monitoring, or dismiss it. Run `npm run discovery:run` to initialise the fixture. CLI scheduling and workers can run independently with `npm run discovery:schedule` and `npm run discovery:worker`.
