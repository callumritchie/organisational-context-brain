...
The first Milestone 8 vertical slice adds a continual hypothesis and memory loop. Relevant source-version changes trigger the same context pipeline under a restricted service identity, compare durable before/after checkpoints, retain evidence deltas, and form attributable memory candidates. Candidates remain proposals until the Project Lead accepts or dismisses them; acceptance promotes a provenance-linked canonical Hypothesis Resource rather than changing model weights or silently rewriting organisational truth.
...
Completed: Milestones 0–6. Milestone 7 scale and messiness validation is in progress; deterministic 10,000-record ingestion, ambiguity preservation, long-document chunks, current-version retirement, replay-safe incremental updates, permission-scoped lexical measurements, two genuine capped semantic samples, and a fail-closed blind-question authoring/validation workflow are implemented. The first Milestone 8 slice now includes event-triggered hypothesis evaluation, durable context checkpoints, evidence deltas, reviewable latent-learning proposals and promotion into provenance-linked canonical memory. Collection of independently authored questions, full-scale vector measurements, general-purpose hypothesis formation, asynchronous workers/schedulers, notification delivery and production monitor operations remain. Deliberately deferred: broader signal producers, approximate vector indexing at scale, and production authentication/deployment hardening.
