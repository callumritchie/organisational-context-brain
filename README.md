...
At the top of that drawer, the **Unprompted discovery** lane shows a separate supplier-onboarding example: unlabeled source records flow through an ontology-guided pattern scan to an explicitly untrusted hypothesis candidate and human review gate. Alex can accept and start monitoring it or dismiss it. Run `npm run discovery:run` to initialise or repeat the idempotent local sweep.
