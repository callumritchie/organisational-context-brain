...
`GET /api/v1/hypotheses` is the first compatibility boundary: it reads the two existing stores through their normal actor-scoped services and returns one projection for product surfaces. Review decisions now enter through one origin-aware `/api/v1/hypotheses/candidates/{origin}/{id}/review` command; pause, resume and manual runs use `/api/v1/hypotheses/operations/{origin}`. Both return the same projection. The adapters deliberately delegate to the existing transaction owners, so queues and persistence tables are not yet merged. Callers can adopt one vocabulary and command shape before durable writes move behind one service.
