...
Milestones 0–6 are complete. Milestone 7 now generates and ingests a deterministic 10,000-record corpus into an isolated workspace whose expected identities, versions, contradictions and permission boundaries are known independently from retrieval. Ambiguous source identities remain explicit candidates rather than being silently merged. The current implementation includes long-document chunking, exact offsets, evidence-level deduplication, retirement of superseded search data, and replay-safe incremental revisions and tombstones with measured throughput. Semantic-sample and exact-pgvector measurements remain before any specialist datastore is selected.
