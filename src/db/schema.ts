...
export const notificationOutbox = pgTable('notification_outbox', {
  ...
  workspaceId: uuid('workspace_id').notNull().references(() => workspaces.id),
  accessScopeId: uuid('access_scope_id').notNull().references(() => accessScopes.id),
  monitorPolicyId: uuid('monitor_policy_id').references(() => monitorPolicies.id),
...
export const hypothesisDiscoveryPolicies = pgTable('hypothesis_discovery_policies', {
  id: uuid('id').primaryKey(),
  workspaceId: uuid('workspace_id').notNull().references(() => workspaces.id),
  accessScopeId: uuid('access_scope_id').notNull().references(() => accessScopes.id),
  ownerActorId: uuid('owner_actor_id').notNull().references(() => users.id),
  ...
  minimumSourceDiversity: integer('minimum_source_diversity').notNull().default(3),
  status: text('status').notNull().default('active'),
...
export const hypothesisDiscoveryCandidates = pgTable('hypothesis_discovery_candidates', {
  id: uuid('id').primaryKey(),
  workspaceId: uuid('workspace_id').notNull().references(() => workspaces.id),
  accessScopeId: uuid('access_scope_id').notNull().references(() => accessScopes.id),
  discoveryPolicyId: uuid('discovery_policy_id').notNull().references(() => hypothesisDiscoveryPolicies.id),
