...
export type OntologyRelationshipEdit = z.infer<
  typeof ontologyRelationshipEditSchema
>;
...
function nextVersion(currentVersion: string) {
  ...
  if (!match)
    throw new OntologyConflictError(
      'The current ontology version cannot be incremented',
    );
  return `${match[1]}-v${Number(match[2]) + 1}`;
}

function toOntologyDto(
  version: string,
  checksum: string,
  ...
  return {
    ...
    status: 'current',
    checksum,
    resourceTypes: Object.entries(document.resourceTypes).map(
      ([name, definition]) => ({ name, ...definition }),
    ),
    relationships: Object.entries(document.relationships).map(
      ([name, definition]) => ({ name, ...definition }),
    ),
    aliases: Object.entries(document.aliases).map(([alias, definition]) => ({
      alias,
      ...definition,
    })),
  };
...
async function publishOntologyRelationship(
  ...
  if (!current)
    throw new OntologyConflictError('No current ontology version is available');
  const document = ontologySchema.parse(current.schema_document);
  if (
    !document.resourceTypes[input.from] ||
    !document.resourceTypes[input.to]
  ) {
    throw new OntologyConflictError(
      'Relationship endpoints must use existing resource types',
    );
  }
  ...
  await client.query(
    ...
    [
      stableId('ontology-version', version),
      IDS.workspace,
      version,
      nextDocument,
      checksum,
...
export async function publishOntologyRelationshipForDemoActor(
  ...
  try {
    ...
    if (
      applied.rows[0]?.actor_id !== actor.id ||
      applied.rows[0]?.workspace_id !== IDS.workspace
    ) {
      throw new Error(
        'Ontology editor transaction scope could not be established',
      );
    }
