...
export const ONTOLOGY = ontologySchema.parse({
  ...
  relationships: {
    ...
    BELONGS_TO: { from: ['ResearchNote', 'MeetingNote', 'Document', 'Evidence'], to: ['Project'], description: 'Knowledge is scoped to a project.' },
    AUTHORED: { from: ['Person'], to: ['ResearchNote', 'MeetingNote', 'Document'], description: 'A person authored content.' },
    DERIVED_FROM: { from: ['Evidence'], to: ['ResearchNote', 'MeetingNote', 'Document'], description: 'Evidence is grounded in content.' },
